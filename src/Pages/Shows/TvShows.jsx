...
import { fetchDiscoverTvShows } from '../../Services/moviesService'
...
const TvShows = () => {
    ...
    useEffect(() => {
        const discoverMovies = async () => {
            try {
                setIsLoading(true)
                const response = await fetchDiscoverTvShows(page, sortby)
    ...
    return (
        ...
        <Container maxW={'1200px'} pt={"8rem"}>

            <Flex alignItems={"center"} gap={"10px"} pb={6} justifyContent={"space-between"} wrap={"wrap"}>
                <Heading as="h2" fontSize="4xl" textTransform="capitalize">tv shows</Heading>
                <Select
                    w={"130px"}
                    onChange={(e) => {
                        setPage(1);
            ...
            >
                {
                    isLoading ? Array.from({ length: 20 }).map((item, index) => <MovieSkeletonCard key={index + 1} />) :
                        movies?.map(movie => <MovieCard key={movie.id} movie={movie} mediaType='tv' />)
                }
            </Grid>
